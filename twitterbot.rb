...
list_journalist.each do |x|  #on itere avec la méthode each do |x| pour appliquer la méthode suivante sur le prochain Array      
    tweet_id = client.user_timeline(x) #Fetch on définit une variable qui comprend les tweet les plus recent (par IDs) des users de la liste avec la méthode client.user_timeline(x) 
